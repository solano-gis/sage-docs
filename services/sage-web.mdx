...
```
Browser (Chat.tsx)
  ↓ user message + map state + screenshot
/api/chat (Next.js route)
  ↓ converts UI messages → model messages
  ├── Injects map screenshot as image part
  ├── Optimizes token budget (strips old images)
  ├── Connects to MCP servers (sage-gov + sage-gis in parallel)
  ↓
Claude Opus 4.6 (with up to 59 tools)
  ├── Reads user question + map screenshot
  ├── Chains 5-8 tool calls per typical query
  ├── Tools execute server-side (MCP) or client-side (map commands)
  ↓
Response streams back to browser
  └── ToolResult.tsx renders maps, data, tool results
```
...
**14 core tools** (~5K tokens) — always available:
- Property: `geocode_address`, `get_parcel_details`, `get_zoning`, `get_flood_zone`, `get_fire_hazard_zone`, `get_elevation`, `get_special_districts`, `find_nearby`
- Maps: `show_map`, `update_map`, `locate_in_view`, `query_map_features`
- Reference: `get_solano_context`, `get_layer_context`
- Meta: `load_tool_group`
