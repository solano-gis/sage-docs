...
- **`highlight-features`** — Highlights any queryable FeatureServer layer by SQL WHERE expression. Geometry-adaptive symbols: orange polygon outlines, orange polylines, or orange circle markers depending on feature type. Capped at 500 features per query.

Examples: `where: "zone_abrev = 'R-1'"` highlights all R-1 zoning districts; `where: "HAZ_CLASS = 'Very High'"` highlights severe fire hazard zones.
