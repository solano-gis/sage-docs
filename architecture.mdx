...
┌─────────────────────────────────────────────────┐
│                   sage-web                       │
│          Chat UI + AI Orchestration              │
│       (Claude Opus 4.6, Vercel AI SDK)          │
└──────┬──────────────┬──────────────┬────────────┘
       │ MCP/HTTP     │ MCP/HTTP     │ iframe postMessage
       ▼              ▼              ▼
┌──────────┐  ┌──────────┐  ┌──────────────────┐
│ sage-gov │  │ sage-gis │  │    sage-map       │
│ 31 tools │  │ 28 tools │  │ ArcGIS Maps SDK  │
│ Records  │  │ Spatial  │  │ Interactive map   │
└──────────┘  └──────────┘  └──────────────────┘
```
...
<AccordionGroup>
  <Accordion title="sage-web — Chat UI + AI orchestration">
    **What it does:** Hosts the chat interface, manages conversations, orchestrates AI tool calls, handles map screenshots, optimizes token budgets.

    **What it accesses:** Only what sage-gov and sage-gis return via MCP. Cannot query databases or GIS services directly. This is the governance surface — authentication, moderation, audit logging, disclaimers, and rate limiting live here.
...
|---------|-----------|-----|
| sage-web | `solano-gis/sage-web` | (private) |
| sage-gis | `solano-gis/sage-gis` | `sage-gis.vercel.app/api/mcp` |
| sage-gov | `solano-gis/sage-gov` | `sage-gov.vercel.app/api/mcp` |
| sage-map | `solano-gis/sage-map` | `sage-map.vercel.app` |

Each repo has its own `CLAUDE.md` with service-specific development context.
